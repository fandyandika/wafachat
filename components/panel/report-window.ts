...
export {
  businessDateKeyForWindowKey, fourPmWibMs, windowKeyFor, windowKeyForBusinessDate,
  windowRangeForKey, windowKeyToday,
} from "@/lib/report-window-core";

/** Closing-date convention: report labelled D covers [4pm (D-1), 4pm D). */
...
/** Closing-date label of the window containing `now`: today before 16:00 WIB, otherwise tomorrow. */
export function currentReportLabelDate(now: number): { y: number; m: number; d: number } {
  ...
  const labelMs = w.getUTCHours() >= 16 ? base + 86_400_000 : base;

...
test("absolute scorecard: the best all-rounder wins", () => {
  const q = computeQueenCs([
    ...
    row("Gamma", 20, 78, 20, 1_080_000, 10), // best CR but low volume + 18min (past the zero-line)
  ]);
  expect(q?.csName).toBe("Alpha");
});

test("speed: <=10 min is perfect (SOP, equal); a growing penalty above 10 min; ~50% at 12.5 min; 0 by 15", () => {
  const s = computeQueenScores([
    row("A", 30, 60, 30, 60_000, 10),    // 1 min — perfect
    row("B", 30, 60, 30, 600_000, 10),   // 10 min (SOP line) — still perfect
    row("C", 30, 60, 30, 750_000, 10),   // 12.5 min (midpoint) — ~half
    row("D", 30, 60, 30, 1_200_000, 10), // 20 min — zero
  ]);
  const by = new Map(s.map((r) => [r.csName, r]));
  expect(by.get("A")!.speedWpts).toBe(15); // perfect
  expect(by.get("B")!.speedWpts).toBe(15); // 10 min still perfect, equal to A (not a race)
  expect(by.get("C")!.speedWpts).toBeCloseTo(7.5, 5); // 50 pts * 0.15 midway between 10 and 15

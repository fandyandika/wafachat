...
test("autoFollowUpSweep: outside hours (02:00 WIB) -> skipped, no send", async () => {
  ...
  await t.run(async (ctx) => {
    convId = await ctx.db.insert("conversations", { ...convBase, orderId: "O-1", customerPhone: "62811" });
    await ctx.db.insert("orders", { ...orderBase, orderId: "O-1", customerPhone: "62811" });
    await ctx.db.insert("messages", msg(convId, "O-1", "62811", "inbound", now - 30 * HOUR));
    await ctx.db.insert("messages", msg(convId, "O-1", "62811", "outbound", now - 29 * HOUR));
    await ctx.db.insert("csConfigs", csCfg("Nabila", true));
  });
...
test("autoFollowUpSweep: within hours, enabled CS, one ghosted H+1 lead -> sends + bumps count", async () => {
  ...
  let convId: any;
  await t.run(async (ctx) => {
    convId = await ctx.db.insert("conversations", { ...convBase, orderId: "O-2", customerPhone: "62812" });
    ...
    await ctx.db.insert("messages", msg(convId, "O-2", "62812", "inbound", now - 30 * HOUR));
    await ctx.db.insert("messages", msg(convId, "O-2", "62812", "outbound", now - 29 * HOUR));
    await ctx.db.insert("csConfigs", csCfg("Nabila", true));
  });
...
test("autoFollowUpSweep: cap reached (autoSentCount = AUTO_DAILY_CAP) -> no send", async () => {
  const t = convexTest(schema);
  await t.run(async (ctx) => {
    const convId = await ctx.db.insert("conversations", { ...convBase, orderId: "O-3", customerPhone: "62813" });
    await ctx.db.insert("orders", { ...orderBase, orderId: "O-3", customerPhone: "62813" });
    await ctx.db.insert("messages", msg(convId, "O-3", "62813", "inbound", now - 30 * HOUR));
    await ctx.db.insert("messages", msg(convId, "O-3", "62813", "outbound", now - 29 * HOUR));
...
test("autoFollowUpSweep: disabled CS (autoFollowUpEnabled false) -> no send", async () => {
  ...
  let convId: any;
  await t.run(async (ctx) => {
    convId = await ctx.db.insert("conversations", { ...convBase, orderId: "O-4", customerPhone: "62814" });
    await ctx.db.insert("orders", { ...orderBase, orderId: "O-4", customerPhone: "62814" });
    await ctx.db.insert("messages", msg(convId, "O-4", "62814", "inbound", now - 30 * HOUR));
    await ctx.db.insert("messages", msg(convId, "O-4", "62814", "outbound", now - 29 * HOUR));
    await ctx.db.insert("csConfigs", csCfg("Nabila", false)); // disabled
  });
...
test("autoFollowUpSweep: disabled CS (autoFollowUpEnabled undefined) -> no send", async () => {
  ...
  let convId: any;
  await t.run(async (ctx) => {
    convId = await ctx.db.insert("conversations", { ...convBase, orderId: "O-5", customerPhone: "62815" });
    await ctx.db.insert("orders", { ...orderBase, orderId: "O-5", customerPhone: "62815" });
    await ctx.db.insert("messages", msg(convId, "O-5", "62815", "inbound", now - 30 * HOUR));
    await ctx.db.insert("messages", msg(convId, "O-5", "62815", "outbound", now - 29 * HOUR));
    ...
    await ctx.db.insert("csConfigs", rest);
  });

...
- Lifecycle status scans remain deliberately bounded and eventual-consistent across query transactions. A handover→active transition can move behind the active boundary after the handover range has already been observed; no row is deleted or lost, and it is processed after the current cycle reaches both terminals and resets.
- The SLA regression simulates that transition, proves the row is absent from the first cycle's collected IDs and remains open, then proves the following cycle considers and closes it. A separate durable transition queue or sweep snapshot could provide same-run capture, but is a future architectural enhancement outside this I/O-remediation scope; a status-independent historical scan would double growing-table reads.
